...
st.set_page_config(page_title="Player Year-over-Year", layout="wide", page_icon="⚾")
...
with title_col:
    st.title("Player Year-over-Year")
with meta_col:
    st.markdown(
        """
...
STAT_PRESETS_YOY = U.STAT_PRESETS_YOY
...
default_name = "Aaron Judge" if is_hitting else "Cam Schlittler"
...
MANUAL_UPDATE_KEY    = f"{prefix}_stat_manual_update"
...
DEFAULT_PRESET = "Statcast" if is_hitting else "Default"
...
with right_col:
    ...
    if is_hitting:
        ...
        with col_btn4:
            min_pa_end = st.number_input(f"Min PA ({int(end_yr)})", min_value=0, max_value=700, value=get_dynamic_min_pa(current_year), step=25)
    else:
        with col_btn3:
            min_ip_start = st.number_input(f"Min IP ({int(start_yr)})", min_value=0, max_value=700, value=100, step=25)
        with col_btn4:
            min_ip_end = st.number_input(f"Min IP ({int(end_yr)})", min_value=0, max_value=700, value=int(get_dynamic_min_ip(current_year)), step=25)
        ...
        if df_start is None or df_end is None:
            ...
            # Apply PA filters before merging
                ...
                if "IP" in df_start.columns:
                    df_start = df_start[pd.to_numeric(df_start["IP"], errors="coerce") >= min_ip_start]
                if "IP" in df_end.columns:
                    df_end = df_end[pd.to_numeric(df_end["IP"], errors="coerce") >= min_ip_end]
