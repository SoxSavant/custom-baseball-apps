...
# Derived stats
if "TB" not in df_all.columns or df_all["TB"].isna().all():
    doubles = pd.to_numeric(df_all.get("2B"), errors="coerce")
    triples = pd.to_numeric(df_all.get("3B"), errors="coerce")
    hr      = pd.to_numeric(df_all.get("HR"), errors="coerce")
    singles = pd.to_numeric(df_all.get("1B"), errors="coerce")
    df_all["TB"] = singles + 2*doubles + 3*triples + 4*hr

if "XBH" not in df_all.columns or df_all["XBH"].isna().all():
    doubles = pd.to_numeric(df_all.get("2B"), errors="coerce")
    triples = pd.to_numeric(df_all.get("3B"), errors="coerce")
    hr      = pd.to_numeric(df_all.get("HR"), errors="coerce")
    df_all["XBH"] = doubles + triples + hr
...
# ─────────────────────────────────────────────
#  Aggregate: count qualifying seasons per player, top 10
# ─────────────────────────────────────────────
